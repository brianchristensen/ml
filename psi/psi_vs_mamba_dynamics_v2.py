"""
PSI vs Mamba - Dynamics Prediction Benchmark V2

Key change: Predict FULL trajectory at once (not autoregressive)
- Input: context of N timesteps
- Output: next M timesteps all at once

This matches how PSI works best.
"""
...
# ============================================================================
# PSI Model - Full Trajectory Prediction
# ============================================================================
...
class PSITrajectoryModel(nn.Module):
    """PSI model that predicts full trajectory at once."""
    def __init__(self, state_dim=3, dim=128, n_layers=4, n_oscillators=64,
                 context_len=20, pred_len=50, max_len=256):
        super().__init__()
        ...
        # Predict full trajectory from final hidden state
        self.head = nn.Sequential(
            nn.Linear(dim, dim * 2),
            nn.GELU(),
            nn.Linear(dim * 2, pred_len * state_dim)
        )

    def forward(self, context):
        """
        context: [B, context_len, state_dim]
        returns: [B, pred_len, state_dim]
        """
        B = context.shape[0]
        h = self.embed(context)
        for norm, block in zip(self.norms, self.blocks):
            h = block(norm(h))

        # Use last position to predict full trajectory
        h_last = self.norm_out(h[:, -1, :])  # [B, dim]
        pred = self.head(h_last)  # [B, pred_len * state_dim]
        return pred.view(B, self.pred_len, self.state_dim)


# ============================================================================
# Mamba Trajectory Model
# ============================================================================

class MambaTrajectoryModel(nn.Module):
    """Mamba model that predicts full trajectory at once."""
    def __init__(self, state_dim=3, dim=128, n_layers=4, d_state=16,
                 context_len=20, pred_len=50):
        super().__init__()
        self.state_dim = state_dim
        self.dim = dim
        self.pred_len = pred_len
...
def train_trajectory_model(model, train_data, epochs, lr, context_len, pred_len, batch_size=64):
    """Train on full trajectory prediction."""
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=0.01)
    criterion = nn.MSELoss()

    n_traj, traj_len, state_dim = train_data.shape
    max_start = traj_len - context_len - pred_len
    ...
    for epoch in range(epochs):
        ...
        for _ in range(100):  # 100 batches per epoch
            ...
            for i in range(batch_size):
                ...
                tgt = train_data[traj_idx[i], start_idx[i]+context_len:start_idx[i]+context_len+pred_len]
                contexts.append(ctx)
                targets.append(tgt)
...
def evaluate_trajectory(model, val_data, context_len, pred_len, n_samples=100):
    """Evaluate full trajectory prediction."""
    model.eval()
    n_traj, traj_len, state_dim = val_data.shape
    max_start = traj_len - context_len - pred_len

    step_errors = np.zeros(pred_len)
    ...
    with torch.no_grad():
        for _ in range(n_samples):
            ...
            gt_future = val_data[traj_idx, start_idx+context_len:start_idx+context_len+pred_len]
...
def main():
    print("=" * 80)
    print("PSI vs MAMBA - FULL TRAJECTORY PREDICTION")
    print("=" * 80)
    ...
    train_data, mean, std = generate_dataset(300, 250)
    val_data, _, _ = generate_dataset(100, 250)
    val_data = (val_data * std) / std  # Already normalized
    ...
    n_layers = 4
    context_len = 20
    pred_len = 50  # Predict 50 steps at once
    epochs = 15
    ...
    # ========== PSI ==========
    print("-" * 80)
    print("PSI (Full Trajectory Prediction)")
    print("-" * 80)
    ...
    for lr in [1e-3, 5e-4]:
        ...
        model = PSITrajectoryModel(
            state_dim, dim, n_layers,
            context_len=context_len, pred_len=pred_len
        ).to(device)
        params = sum(p.numel() for p in model.parameters())

        start_time = time.time()
        train_trajectory_model(model, train_data, epochs, lr, context_len, pred_len)
        train_time = time.time() - start_time
        train_mem = get_gpu_memory_mb()

        step_errors = evaluate_trajectory(model, val_data, context_len, pred_len)
    ...
    print()
    print("-" * 80)
    print("Mamba (Full Trajectory Prediction)")
    print("-" * 80)
    ...
    for cfg in mamba_configs:
        ...
        model = MambaTrajectoryModel(
            state_dim, dim, n_layers, d_state=cfg['d_state'],
            context_len=context_len, pred_len=pred_len
        ).to(device)
        params = sum(p.numel() for p in model.parameters())

        start_time = time.time()
        train_trajectory_model(model, train_data, epochs, cfg['lr'], context_len, pred_len)
        train_time = time.time() - start_time
        train_mem = get_gpu_memory_mb()
    ...
    print()
    print("=" * 80)
    print("SUMMARY: Lorenz Full Trajectory Prediction (50 steps at once)")
    print("=" * 80)
    print()
    ...
    print("Error at specific prediction steps:")
    ...
    if psi_wins > mamba_wins:
        print("\nVERDICT: PSI outperforms Mamba on full trajectory prediction!")
    elif mamba_wins > psi_wins:
        print("\nVERDICT: Mamba outperforms PSI on full trajectory prediction")
    else:
        print("\nVERDICT: PSI and Mamba are comparable on full trajectory prediction")
