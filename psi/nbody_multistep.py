"""
N-body Multi-step PSI Benchmark

Tests whether multi-step prediction (predicting K future states at once)
improves N-body dynamics over single-step autoregressive prediction.

Key insight: Instead of predicting one state at a time (slow, error accumulation),
predict K states at once and integrate via cumsum.
"""
...
class NBodyMultiStepDataset(Dataset):
    ...
    def __init__(self, trajectories, context_len=20, K=10, split='train', train_frac=0.8):
        n_bodies = trajectories.shape[2]
        state_features = []
        for i in range(n_bodies):
            state_features.append(trajectories[:, :, i, 1:])  # Skip mass
        ...
        # Need context + K future states
        self.seqs_per_traj = self.trajectories.shape[1] - context_len - K
    ...
    def __getitem__(self, idx):
        ...
        # Return K future states instead of just 1
        future = trajectory[pos + self.context_len:pos + self.context_len + self.K]

        return {
            'context': torch.tensor(context, dtype=torch.float32),
            'future': torch.tensor(future, dtype=torch.float32)  # [K, state_dim]
        }
...
class PSISingleStep(nn.Module):
    """PSI model that predicts one state at a time (baseline)."""
    ...
    def rollout(self, context, num_steps):
        """Sequential rollout - one step at a time."""
        predictions = []
...
class PSIMultiStep(nn.Module):
    ...
    Key insight: Instead of predicting states directly, predict derivatives
    and integrate via cumsum. This reduces error accumulation.
    """

    def __init__(self, state_dim, dim=128, num_layers=6, K=10, max_len=50):
        super().__init__()
        self.state_dim = state_dim
        self.dim = dim
        self.K = K
    ...
    def forward(self, states):
        """
        Args:
            states: [batch, seq_len, state_dim] context

        Returns:
            future: [batch, K, state_dim] next K states
        """
        batch_size, seq_len, _ = states.shape
        ...
        # Get final hidden state and expand for K future steps
        final_hidden = x[:, -1:, :]  # [batch, 1, dim]

        # Add time embeddings for K future steps
        h = final_hidden.expand(-1, self.K, -1) + self.time_embed.expand(batch_size, -1, -1)

        # Process through PSI blocks for temporal coherence
        for block in self.blocks:
            h = block(h)

        # Predict derivatives
        dx = self.derivative_head(h)  # [batch, K, state_dim]

        # Integrate via cumsum from last known state
        last_state = states[:, -1, :]  # [batch, state_dim]
        future = last_state.unsqueeze(1) + torch.cumsum(dx * self.dt_scale, dim=1)

        return future  # [batch, K, state_dim]

    def rollout(self, context, num_steps):
        """
        Rollout by chaining K-step predictions.
        Much faster than sequential single-step rollout.
        """
        predictions = []
        current_context = context.clone()
        ...
        while steps_remaining > 0:
            # Predict K steps at once
            future = self.forward(current_context)  # [batch, K, state_dim]

            take = min(self.K, steps_remaining)
            predictions.append(future[:, :take, :])

            # Update context with last predicted states
            if take < self.K:
                new_states = future[:, :take, :]
            else:
                new_states = future
        ...
        return torch.cat(predictions, dim=1)  # [batch, num_steps, state_dim]
...
class LSTMSingleStep(nn.Module):
    """Manual LSTM for single-step prediction (baseline)."""
...
def train_single_step(model, train_loader, val_loader, epochs=50, lr=1e-4):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            target = future[:, 0, :]  # Only first future state
...
def train_multi_step(model, train_loader, val_loader, epochs=50, lr=1e-4):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            context = batch['context'].to(device)
            future = batch['future'].to(device)  # [batch, K, state_dim]

            optimizer.zero_grad()
            pred = model(context)  # [batch, K, state_dim]
...
def evaluate_rollout(model, test_trajectories, context_len, mean, std, n_bodies, rollout_steps=30):
...
def main():
    print("=" * 80)
    print("N-BODY MULTI-STEP PSI BENCHMARK")
    print("=" * 80)
    ...
    n_bodies = 3
    trials = 500
    timesteps = 100  # Increased to accommodate K=rollout_steps
    context_len = 20
    rollout_steps = 30
    K = rollout_steps  # Single forward pass for entire rollout!
    dim = 128
    num_layers = 6
    epochs = 50
    batch_size = 128
    ...
    # Models
    models = {
        'PSI Single-Step': PSISingleStep(state_dim, dim, num_layers, context_len).to(device),
        f'PSI Multi-Step (K={K})': PSIMultiStep(state_dim, dim, num_layers, K, context_len).to(device),
        'LSTM Single-Step': LSTMSingleStep(state_dim, dim, num_layers).to(device),
    }
    ...
    for name, model in models.items():
        ...
        results[name] = {
            ...
            'best_val': best_val,
            'rollout_mse': mse_mean,
            'rollout_mse_std': mse_std,
            'rollout_time': time_mean,
            'rollout_time_std': time_std,
            'history': history
        }

        print(f"\n  Training time:     {train_time:.1f}s")
        print(f"  Best val loss:     {best_val:.6f}")
        print(f"  Rollout MSE:       {mse_mean:.6f} (+/- {mse_std:.6f})")
        print(f"  Rollout time:      {time_mean*1000:.2f} (+/- {time_std*1000:.2f}) ms")
    ...
    print(f"\n{'Model':<25} {'Params':>10} {'Train':>8} {'Rollout MSE':>14} {'Time (ms)':>12}")
    print("-" * 80)

    for name, r in results.items():
        print(f"{name:<25} {r['params']:>10,} {r['train_time']:>7.1f}s {r['rollout_mse']:>14.6f} {r['rollout_time']*1000:>12.2f}")

    # Multi-step vs Single-step improvement
    print("\n" + "=" * 80)
    print("MULTI-STEP IMPROVEMENT")
    print("=" * 80)

    psi_single = results['PSI Single-Step']
    psi_multi = results[f'PSI Multi-Step (K={K})']
    lstm_single = results['LSTM Single-Step']

    # Accuracy improvement
    accuracy_vs_psi = (psi_single['rollout_mse'] - psi_multi['rollout_mse']) / psi_single['rollout_mse'] * 100
    accuracy_vs_lstm = (lstm_single['rollout_mse'] - psi_multi['rollout_mse']) / lstm_single['rollout_mse'] * 100

    # Speed improvement
    speedup_vs_psi = psi_single['rollout_time'] / psi_multi['rollout_time']
    speedup_vs_lstm = lstm_single['rollout_time'] / psi_multi['rollout_time']

    print(f"\nPSI Multi-Step vs PSI Single-Step:")
    print(f"  Accuracy:  {accuracy_vs_psi:+.1f}%")
    print(f"  Speed:     {speedup_vs_psi:.2f}x faster")

    print(f"\nPSI Multi-Step vs LSTM Single-Step:")
    print(f"  Accuracy:  {accuracy_vs_lstm:+.1f}%")
    print(f"  Speed:     {speedup_vs_lstm:.2f}x faster")

    # Plot
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))

    colors = {'PSI Single-Step': 'steelblue', f'PSI Multi-Step (K={K})': 'coral', 'LSTM Single-Step': 'seagreen'}
    ...
    # Rollout accuracy comparison
    ax = axes[0, 1]
    names = list(results.keys())
    mses = [results[n]['rollout_mse'] for n in names]
    stds = [results[n]['rollout_mse_std'] for n in names]
    bars = ax.bar(range(len(names)), mses, yerr=stds, color=[colors[n] for n in names], capsize=5)
    ax.set_xticks(range(len(names)))
    ax.set_xticklabels([n.replace(' ', '\n') for n in names], fontsize=9)
    ax.set_ylabel(f'{rollout_steps}-step Rollout MSE')
    ax.set_title('Rollout Accuracy')
    ax.grid(True, alpha=0.3, axis='y')

    # Rollout time comparison
    ax = axes[1, 0]
    times = [results[n]['rollout_time'] * 1000 for n in names]
    time_stds = [results[n]['rollout_time_std'] * 1000 for n in names]
    bars = ax.bar(range(len(names)), times, yerr=time_stds, color=[colors[n] for n in names], capsize=5)
    ax.set_xticks(range(len(names)))
    ax.set_xticklabels([n.replace(' ', '\n') for n in names], fontsize=9)
    ax.set_ylabel('Rollout Time (ms)')
    ax.set_title(f'{rollout_steps}-step Rollout Speed')
    ax.grid(True, alpha=0.3, axis='y')

    # Speed vs Accuracy
    ax = axes[1, 1]
    for name, r in results.items():
        ax.scatter(r['rollout_time']*1000, r['rollout_mse'], s=150, c=colors[name], label=name, zorder=3)
    ax.set_xlabel('Rollout Time (ms)')
    ...
    plt.tight_layout()
    plt.savefig('nbody_multistep.png', dpi=150)
    plt.close()
    print("\nSaved nbody_multistep.png")
