"""
Test: Small hidden dimension (8D instead of 128D)

Hypothesis: Smaller hidden dim = fewer params to corrupt = less forgetting.
Also tests whether the phase memory mechanism needs large hidden dims.
"""
...
class SmallBlock(nn.Module):
    ...
    def __init__(self, dim, n_orthogonal_sets=2, planes_per_set=4, pos_planes=4):
        super().__init__()
        self.dim = dim
...
class SmallHiddenModel(nn.Module):
    """Model with small hidden dimension"""

    def __init__(self, input_dim=3, hidden_dim=8, n_layers=4,
                 n_orthogonal_sets=2, planes_per_set=4, pos_planes=4,
        ...
        self.blocks = nn.ModuleList([
            SmallBlock(hidden_dim, n_orthogonal_sets, planes_per_set, pos_planes=pos_planes)
...
def run_experiment():
    print("=" * 70)
    print("SMALL HIDDEN DIMENSION TEST")
    print("=" * 70)
    ...
    configs = [
        # (name, hidden_dim, input_mode, output_mode)
        ("128D Full+Ortho (baseline)", 128, "full", "linear_ortho"),
        ("8D Full+Ortho", 8, "full", "linear_ortho"),
        ("8D Full+Full", 8, "full", "full"),
        ("8D Ortho+Ortho", 8, "linear_ortho", "linear_ortho"),
        ("16D Full+Ortho", 16, "full", "linear_ortho"),
        ("32D Full+Ortho", 32, "full", "linear_ortho"),
    ]

    for name, hidden_dim, input_mode, output_mode in configs:
        print(f"\n{'='*60}")
        print(f"Testing: {name}")
        print('='*60)

        # Adjust planes for smaller hidden dim
        if hidden_dim <= 16:
            n_sets, planes_per_set, pos_planes = 2, 4, 4
        elif hidden_dim <= 32:
            n_sets, planes_per_set, pos_planes = 2, 8, 8
        else:
            n_sets, planes_per_set, pos_planes = 4, 16, 16

        model = SmallHiddenModel(
            input_dim=3, hidden_dim=hidden_dim, n_layers=4,
            n_orthogonal_sets=n_sets, planes_per_set=planes_per_set,
        ...
        print(f"  Hidden dim: {hidden_dim}, Sets: {n_sets}, Planes: {planes_per_set}")
    ...
    # Summary
    print("\n" + "=" * 70)
    print("SUMMARY: HIDDEN DIMENSION COMPARISON")
    print("=" * 70)
    print(f"{'Config':<30} {'Dim':>5} {'Params':>10} {'Lorenz A':>12} {'Lorenz B':>12} {'Forget':>10}")
    print("-" * 82)

    for name, r in sorted(results.items(), key=lambda x: x[1]['forgetting']):
        print(f"{name:<30} {r['hidden_dim']:>5} {r['trainable']:>10,} {r['lorenz_A']:>12.6f} {r['lorenz_B']:>12.6f} {r['forgetting']:>+9.0f}%")
