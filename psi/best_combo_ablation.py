"""
Test best combination: fix content_pos_gate + fix output_proj

These two showed lowest forgetting while maintaining learning ability.
"""
...
class OptimizedBlock(nn.Module):
    """Block with fixed content_pos_gate, trainable everything else"""
    ...
    def __init__(self, dim, n_orthogonal_sets=4, planes_per_set=4, pos_planes=16):
        ...
        # Fixed key/query projections (from previous findings)
        ...
        # FIXED content_pos_gate (just 0.5 blend)
...
class OptimizedModel(nn.Module):
    """Model with fixed output_proj and fixed content_pos_gate"""

    def __init__(self, input_dim=3, hidden_dim=128, n_layers=4,
                 n_orthogonal_sets=4, planes_per_set=16, pos_planes=16,
                 output_mode='full'):
        # output_mode: 'full' (NN), 'fixed' (random), 'linear' (single layer)
        super().__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim

        # Trainable input projection (essential for learning)
        ...
        if output_mode == 'fixed':
            ...
            # Single linear layer - minimal trainable params
            self.output_proj = nn.Linear(hidden_dim, input_dim)

        elif output_mode == 'fourier':
            # Fixed Fourier basis projection
            # Create frequency basis that captures different scales
            freqs = torch.exp(torch.linspace(0, -math.log(1000), hidden_dim // input_dim))
            # Build projection matrix from Fourier components
            fourier_mat = torch.zeros(hidden_dim, input_dim)
            for i in range(input_dim):
                for j in range(hidden_dim):
                    # Mix of sin/cos at different frequencies
                    freq_idx = j // input_dim
                    if freq_idx < len(freqs):
                        if j % 2 == 0:
                            fourier_mat[j, i] = math.cos(2 * math.pi * freqs[freq_idx] * (i + 1))
                        else:
                            fourier_mat[j, i] = math.sin(2 * math.pi * freqs[freq_idx] * (i + 1))
            fourier_mat = fourier_mat / math.sqrt(hidden_dim)
            self.register_buffer('output_proj_mat', fourier_mat)
            # Learnable scale per output dim
            self.output_scale = nn.Parameter(torch.ones(input_dim))
            self.output_proj = lambda x: (x @ self.output_proj_mat) * self.output_scale

        elif output_mode == 'hyperplane':
            # Fixed random hyperplanes with learnable thresholds/scales
            # The hyperplanes are fixed, but we learn how to combine their outputs
            hyperplanes = torch.randn(hidden_dim, hidden_dim // 4) / math.sqrt(hidden_dim)
            self.register_buffer('hyperplanes', hyperplanes)
            # Learnable combination of hyperplane activations to output
            self.hyperplane_to_out = nn.Linear(hidden_dim // 4, input_dim)
            self.output_proj = lambda x: self.hyperplane_to_out(torch.tanh(x @ self.hyperplanes))

        elif output_mode == 'direct_stats':
            # Use statistics of hidden state directly
            # Compute global stats over hidden dim, then project
            # 4 stats (mean, std, max, min) -> learnable combination
            self.stat_weights = nn.Linear(4, input_dim)

            def stats_proj(x):
                B, L, D = x.shape
                # Compute statistics across hidden dimension
                means = x.mean(dim=-1, keepdim=True)
                stds = x.std(dim=-1, keepdim=True)
                maxs = x.max(dim=-1, keepdim=True).values
                mins = x.min(dim=-1, keepdim=True).values
                # Concatenate stats: [B, L, 4]
                stats = torch.cat([means, stds, maxs, mins], dim=-1)
                return self.stat_weights(stats)

            self.output_proj = stats_proj

        elif output_mode == 'linear_ortho':
            # Single linear layer initialized with orthogonal weights
            # Orthogonal init often helps with gradient flow and generalization
            self.output_linear = nn.Linear(hidden_dim, input_dim)
            nn.init.orthogonal_(self.output_linear.weight)
            self.output_proj = self.output_linear
        ...
        self.blocks = nn.ModuleList([
            OptimizedBlock(hidden_dim, n_orthogonal_sets, planes_per_set, pos_planes=pos_planes)
...
def run_experiment():
    print("=" * 70)
    print("BEST COMBO: fix content_pos_gate + fix output_proj")
    print("=" * 70)
    ...
    for name, output_mode in [("Full NN output_proj", "full"),
                               ("Fixed random output_proj", "fixed"),
                               ("Single linear output_proj", "linear"),
                               ("Linear + orthogonal init", "linear_ortho"),
                               ("Fixed Fourier basis + scale", "fourier"),
                               ("Fixed hyperplanes + linear", "hyperplane"),
                               ("Direct statistics", "direct_stats")]:
        print(f"\n{'='*60}")
    ...
    print("  " + "-" * 50)
    print("  Normalized + NN encoders:            +10781%")
    print("  No-norm + NN encoders:               +1850%")
    print("  No-norm + Fixed key/query:           +927%")
    best = results.get("Best combo (fixed output_proj + fixed gate)", {})
    if best:
        print(f"  Best combo (this test):              {best['forgetting']:+.0f}%")
