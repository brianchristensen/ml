"""
Text Generation from Trained TinyStories Model

Usage:
    python generate_tinystory.py
    python generate_tinystory.py --checkpoint tinystories_best.pt --prompt "Once upon a time"
    python generate_tinystory.py --prompt "There was a little girl" --temperature 0.9
"""
...
def generate_text(model, tokenizer, prompt, max_length=150, temperature=0.8, top_k=50, device='cuda'):
    """
...
def interactive_generation(model, tokenizer, device='cuda'):
    """Interactive generation loop for stories."""
    print("=" * 80)
    print("Interactive Story Generation")
    print("=" * 80)
    print()
    print("Commands:")
    print("  Type a prompt and press Enter to generate a story")
    ...
    print()
    print("Story prompts to try:")
    print("  - Once upon a time")
    print("  - There was a little")
    print("  - One day, a [animal]")
    print("  - The [character] was happy because")
    print()
...
def batch_generation(model, tokenizer, prompts, output_file=None,
                     max_length=150, temperature=0.8, top_k=50, device='cuda'):
    """
...
def main():
    parser = argparse.ArgumentParser(description="Generate stories from trained TinyStories model")
    parser.add_argument('--checkpoint', type=str, default='tinystories_best.pt',
                       help='Path to model checkpoint')
    parser.add_argument('--prompt', type=str, default=None,
                       help='Story prompt (if not provided, enters interactive mode)')
    ...
    parser.add_argument('--temperature', type=float, default=0.8,
                       help='Sampling temperature (0.5=safe, 1.0=creative)')
    ...
    if not Path(args.checkpoint).exists():
        ...
        print()
        print("Train a model first with: python train_tinystories.py")
        return
