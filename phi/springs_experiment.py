...
class PHIDynamicsPredictor(nn.Module):
    """PHI model for particle dynamics prediction."""
    ...
    def __init__(self, state_dim, dim=256, num_layers=6, max_len=100, dropout=0.1):
        ...
        self.blocks = nn.ModuleList([PHIBlock(dim=dim) for _ in range(num_layers)])
...
def visualize_trajectories(gt_traj, pred_traj, n_particles, title, filename):
    ...
    axes[1].set_title('PHI Predictions')
    axes[1].set_xlabel('X')
    axes[1].set_ylabel('Y')
    axes[1].legend()
    ...
    axes[2].set_title('Overlay (solid=GT, dashed=PHI)')
    axes[2].set_xlabel('X')
    axes[2].set_ylabel('Y')
...
def main():
    ...
    parser.add_argument('--num_layers', type=int, default=6, help='Number of PHI layers')
    ...
    print("=" * 80)
    print(f"PHI {args.system.upper()} Experiment - NRI Benchmark Comparison")
    print("=" * 80)
    print()
    ...
    print(f"\nPHI Results ({args.system}, {args.n_particles} particles):")
    print("-" * 50)
    ...
    print(f"\n{'Horizon':<10} {'NRI':<15} {'PHI':<15} {'Ratio (PHI/NRI)':<15}")
    print("-" * 55)
    for horizon in [1, 10, 20]:
        if horizon in mse_results_norm:
            nri = nri_baselines[horizon]
            phi = mse_results_norm[horizon]
            ratio = phi / nri
            print(f"{horizon:<10} {nri:<15.2e} {phi:<15.2e} {ratio:<15.1f}x")
