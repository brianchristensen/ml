...
def predict_trajectory(model, initial_states, num_steps, mean, std, device):
    """Rollout PHI predictions autoregressively."""
    model.eval()
    predictions = []
...
def visualize_nbody(ground_truth, phi_predictions, n_bodies, sample_idx=0):
    ...
    axes[1].set_title('PHI Predictions (No Solver!)')
    axes[1].set_xlabel('X')
    axes[1].set_ylabel('Y')
    axes[1].legend()
    ...
    for i in range(n_bodies):
        ...
        axes[2].plot(pred_x, pred_y, color=colors[i % len(colors)], alpha=0.8, linewidth=2, linestyle='--', label=f'PHI {i+1}')
...
def main():
    ...
    print("=" * 80)
    print(f"PHI {args.n_bodies}-Body Gravitational Dynamics Experiment")
    print("=" * 80)
    print()
    ...
    # Create model
    model = PHIDynamicsPredictor(state_dim, args.dim, args.num_layers, args.context_len, device).to(device)
